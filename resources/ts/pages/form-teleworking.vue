...
import type { Employee, TeleworkingFormData } from '@/types'
import SignaturePad from '@/components/SignaturePad.vue'

// Force TypeScript recompilation
const TS_VERSION = Date.now()
...
const formData = ref<TeleworkingFormData>({
  employeeId: '',
  nama_lengkap: '', // Added this field
  nomor_ktp: '', // Added this field for KTP number
  email: '',
  username_vpn: '', // Required by EmployeeFormData
  posisi_jabatan: '',
  nama_organisasi: '',
  nomer_hp_wa: '',
  position: '',
...
const fetchEmployees = async () => {
  try {
    ...
    if (data.success) {
      ...
      console.log('Available employees for dropdown:', employees.value.map((emp: Employee) => ({
        id: emp.id,
...
watch(() => formData.value.employeeId, (newEmployeeId) => {
  ...
  if (newEmployeeId) {
    const selectedEmployee = employees.value.find((emp: Employee) => emp.id === Number(newEmployeeId))
    console.log('Selected employee:', selectedEmployee)
    console.log('Available employees:', employees.value.map((emp: Employee) => ({ id: emp.id, name: emp.nama_lengkap })))
    
    if (selectedEmployee) {
      // Force update using nextTick
      nextTick(() => {
        formData.value.nama_lengkap = selectedEmployee.nama_lengkap // Added this line
        formData.value.nomor_ktp = selectedEmployee.nomor_ktp // Add KTP number
        ...
        formData.value.manager = selectedEmployee.nomer_hp_wa // Auto-fill phone number
      ...
      console.warn('Employee not found for ID:', newEmployeeId)
    }
  } else {
    // Reset employee fields when no employee selected
    nextTick(() => {
      formData.value.nama_lengkap = '' // Added this line
      formData.value.nomor_ktp = '' // Reset KTP number
      formData.value.email = ''
      formData.value.position = ''
      formData.value.department = ''
      formData.value.manager = ''
      console.log('Employee data reset')
...
const submitForm = async () => {
  ...
  try {
    // Pending request validation removed to allow multiple submissions
    ...
    const apiUrl = isEditMode.value ? `/api/teleworking-requests/${editId.value}` : '/api/teleworking-requests'
    ...
    if (formData.value.signature_image) {
      ...
      if (formData.value.signature_image instanceof File) {
        ...
        const base64Data = formData.value.signature_image.split(',')[1]
    ...
    if (result.success) {
      ...
      console.log('Adding notification:', { formType: 'teleworking', email, requestIdStr, dbId, actionText })
      addFormSubmissionNotification('teleworking', email, requestIdStr, actionText)
      ...
      if (result.message?.includes('already has a pending teleworking request')) {
        userMessage = result.message || 'Terjadi kesalahan saat submit form'
      } else if (result.errors?.employee_id?.[0]?.includes('already has a pending request')) {
        userMessage = result.errors?.employee_id?.[0] || 'Terjadi kesalahan saat submit form'
      }
...
const loadRequestData = async (id: number) => {
  try {
    loading.value = true
    console.log('Loading teleworking request data for ID:', id)
    
    const response = await fetch(`/api/teleworking-requests/${id}`)
    ...
    if (result.success) {
      ...
      formData.value.nama_lengkap = request.employee?.nama_lengkap || ''
      formData.value.nomor_ktp = request.employee?.nomor_ktp || ''
...
onMounted(() => {
  ...
  if (editParam) {
    ...
    // Load existing data after fetching employees
    fetchEmployees().then(() => {
      if (editId.value) {
  ...
  <div>
    <VCard>
      <VCardTitle class="d-flex align-center gap-2 pa-4">
        ...
        <span v-if="!isEditMode">Form Teleworking</span>
        <span v-else>Edit Teleworking (ID: {{ editId }})</span>
      </VCardTitle>
